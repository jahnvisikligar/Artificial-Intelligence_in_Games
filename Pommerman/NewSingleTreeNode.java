...
public class NewSingleTreeNode
{
    ...
    NewSingleTreeNode(MCTSParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private NewSingleTreeNode(MCTSParams p, NewSingleTreeNode parent, int childIdx, Random rnd, int num_actions,
...
//        }
    ...
    private void roll(GameState gs, Types.ACTIONS act)
    {
        ...
        for(int i = 0; i < nPlayers; ++i)
        {
            if(playerId == i)
            {
                ...
                int[] opponentPosition = Opponent_Pos(i, gs);
    ...
    private int[] Opponent_Pos(int agentIndex, GameState state){
    ...
    private boolean safeAction(int posX, int posY, Vector2d dir, GameState gs){
        ...
        int w = board.length; //width
        int h = board[0].length; //height
